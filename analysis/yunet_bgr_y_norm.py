...
if __name__ == '__main__':
    ...
    for sub_dir, dirs, filenames in os.walk(main_dir, topdown=True):
        ...
        for filename in filenames:
            ...
            bgr_norm = cv2.merge([b_norm, g_norm, r_norm])
            image_YCrCb = cv2.cvtColor(bgr_norm, cv2.COLOR_BGR2YCrCb)
            image_YCrCb[:,:,0] = cv2.normalize(image_YCrCb[:,:,0], None, alpha = 0, beta = 255, norm_type = cv2.NORM_MINMAX)
            image_norm = cv2.cvtColor(image_YCrCb, cv2.COLOR_YCrCb2BGR)
